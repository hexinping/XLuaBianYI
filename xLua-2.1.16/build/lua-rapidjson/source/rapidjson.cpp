...
int json_encode_to_str(lua_State* L, int Index, uint8_t** buffer, int* bufferSize, int* realSize) {
    Encoder encode(L, Index);
    StringBuffer s;
    encode.encode(L, &s, Index);

	int needSize =  s.GetSize();
	if (needSize > *bufferSize)
	{
		std::free(*buffer);
		*bufferSize = 2 * needSize;
		uint8_t* t  = ((uint8_t*)malloc(*bufferSize));
		*buffer = t;
	}
	*realSize = needSize;
	std::memcpy(*buffer, s.GetString(), needSize);
	return 0;
}
...
int json_encode_to_cb(lua_State* L, int opt, int Index, uint8_t* arg2, int arg2cnt, ON_REUSE_CB cb)
{
	try {
		if (opt == 1)
		{
            ...
            StringBuffer s;
            encode.encode(L, &s, Index);
            cb((uint8_t*)(s.GetString()), s.GetSize(), arg2, arg2cnt);
		}
		else
		{
			const char* tt = " ";
            cb((uint8_t*)(tt), strlen(tt), arg2, arg2cnt);
		}
